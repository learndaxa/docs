...
:::tip[Learn more]
See [TaskGraph](/wiki/taskgraph/) for the full picture: how tasks declare their resource uses, how the graph derives barriers/layout transitions automatically, and persistent vs. transient task resources.
:::
...
:::tip[Learn more]
This `begin_renderpass`/`end_renderpass` pair, `RenderAttachmentInfo`, and `set_pipeline`/`push_constant`/`draw` are the same building blocks used for manual recording in [Command Recording & Submission](/wiki/command-recording/#raster-pass) - TaskGraph just calls them for you inside the task callback. See [Pipelines & Renderpasses](/wiki/pipelines-and-renderpasses/) for the full `RenderAttachmentInfo` (load/store ops, clear values, depth attachments).
:::
