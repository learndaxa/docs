...
<TabItem label="Debian/Ubuntu">
    ```bash
    sudo apt update
    sudo apt install ninja-build clang cmake git pkg-config libwayland-dev libxkbcommon-dev xorg-dev
    ```
</TabItem>
</Tabs>

The template builds [GLFW](https://www.glfw.org/) from source, which on Linux needs the Wayland, xkbcommon and X11 development packages (`libwayland-dev`, `libxkbcommon-dev` and `xorg-dev` above). For other distributions, see GLFW's [dependency list](https://www.glfw.org/docs/latest/compile.html#compile_deps_wayland).
